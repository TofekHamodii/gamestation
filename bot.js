...
client.on('ready', function(){    
    var ms = 100000 ;
    var prefix = "-"; 
    var setGame = [`GamesStation`,`Beta 0.2`,`Road To ,2k`];
    var i = -1;    
    var j = 0;    
...
client.channels.get("506535469685932042").send(`:white_check_mark: **${client.user.tag} دخل سيرفر جديد
...
client.on('guildDelete', guild => {
  client.channels.get("506535469685932042").send(`:negative_squared_cross_mark: **${client.user.tag} طلع من سيرفر
Server name: __${guild.name}__
Server owner: __${guild.owner}__
Server id: __${guild.id}__ 
Server Count: __${guild.memberCount}__**`)
...
if (message.content.startsWith(prefix + 'trans')) {
    ...
    if (!args[0]) {
        ...
        const embed = new Discord.RichEmbed()
            .setColor("FFFFFF")
            .setDescription("**ترجمة الكتابة.**\استعمل: `-trans <الكلمة لتبي> <االغة>`");
        ...
        if (args.length === undefined) {
            ...
            return message.channel.send("**ترجمة الكتابة.**\استعمل: `-trans <الكلمة لتبي> <االغة>`");
...
//xRGRx .. By Julian
  client.on('messageUpdate', (oldJulian, newJulian) => {
   if (newJulian.content.toUpperCase().match(/DISCORD.GG/i))
    {
        console.log(newJulian.author.name + " حاول النشر عبر تعديل الرسآلة - " + newJulian);
           newJulian.delete().catch(O_o=>{}); 
           newJulian.reply("** No Invite Links :angry: !**");
    }
});
  //xRGRx .. By Julian
client.on("message", message => {
...
❖-servavatar ~ صورة السيرفر
❖-mb ~ يعرض لك عدد كل حالات الاشخاص وعدد البوتات وعدد الاشخاص
❖-id ~ معلومات عنك
❖-bans ~ عدد الاشخاص المبندة 
❖-at ~ كتابة محتوى في انجاز ماين كرافتي
❖-mcstats ~ معرفة معلومات عن السيرفر في ماين كرافت
❖-avatar ~ صورتك 
...
❖-mvall ~  لسحب جميع الاعضاء الى روومك
...
❖-ban @user <time> <reason> ~ حضر الشخص من السيرفر
...
❖-cc <number> ~ ينشا لك الوان مع كم الوان تبي
❖-giveaway ~ لبدأ قيف اواي
`)
   message.author.sendEmbed(embed)
...
client.on('message', message => { 
    ...
    .setFooter(`xRGRx`, 'https://images-ext-2.discordapp.net/external/JpyzxW2wMRG2874gSTdNTpC_q9AHl8x8V4SMmtRtlVk/https/orcid.org/sites/default/files/files/ID_symbol_B-W_128x128.gif')                                 
...
if(!message.channel.guild) return message.reply(':no_entry: | This Command For Servers Only!'); 
  ...
  if (message.content === 'jjoin') {
	   if (message.author.id !== '452379478618800129') return message.reply(':no_entry: | This Command Only For Julian :fire: !')
...
if (message.content.startsWith('-sbcall')){
	  if(!message.channel.guild) return message.reply(':no_entry: | This Command For Servers Only!');
 if (message.author.id !== '452379478618800129') return message.reply(':no_entry: | This Command Only For Bot Owner!')
 if(!message.author.id === '452379478618800129') return;
...
client.on('message', message => {
  if (!message.content.startsWith(prefix)) return;
  const verifed = ["452379478618800129"];
